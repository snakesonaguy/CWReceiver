...
class HealthInsights:
    ...
    def parse_alert_body(self, body):
        ...
        try:
            results = list()
            s = body['series']
            columns_list = [c for c in s[0]['columns']]
            # print("COLUMNS {}\n".format(columns_list))
            for v1 in s[0]['values']:
                values_list = [v for v in v1]
                # print("VALUES {}\n".format(values_list))
                z_dict = self.my_zip(columns_list, values_list)
                add_dict = {
                            'id': z_dict.get('id', 'Unknown'),
                            'kpi_id': s[0]['tags'].get('kpi_id', 'Unknown'),
                            'level': s[0]['tags'].get('level', 'Unknown'),
                            'msg': z_dict.get('msg', 'Unknown'),
                            'producer': s[0]['tags'].get('Producer', 'Unknown'),
                            'state': s[0]['tags'].get('state', 'Unknown'),
                            'time': z_dict.get('time', 'Unknown'),
                            'uuid': s[0]['tags'].get('UUID', 'Unknown')
                            }
